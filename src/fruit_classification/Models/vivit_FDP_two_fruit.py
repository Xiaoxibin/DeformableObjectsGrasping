...
class VIVIT(nn.Module):
    def __init__(self,Tactile_img_size, Visual_img_size, Tactile_patch_size, Visual_patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio,qkv_bias,qk_scale, drop_rate, attn_drop_rate, drop_path_rate,num_frames,dropout, use_gpu=False):
        ...
        self.fusion = nn.Linear(4*embed_dim,512)
        self.fc1 = nn.Linear(512, 128)
        self.fc2 = nn.Linear(128, 64)
        #Classifier head
        self.head = nn.Linear(64,num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self,x1,x2,y1,y2,thresh):
        ...
        pred = self.fc1(fused) 
        pred = self.fc2(pred) 
        fruit_pred = self.head(pred)
    
        return fruit_pred

    def fruit_inference(self,x):
        x = self.fusion(x)
        
        fruit_pred = self.fc1(x) 
        fruit_pred = self.fc2(fruit_pred) 
        fruit_pred = self.head(fruit_pred)
