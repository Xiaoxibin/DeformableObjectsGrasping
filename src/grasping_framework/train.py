...
from utils import thresh_data_loader
...
def train_net(params):
    ...
    if 'save_dir' in params.keys():
        ...
        # 创建了一个路径，名为 exp_save_dir = ./'save_dir'/'Model_name'/time
    ...
    ############################################################################ Dataloader
    # train_dataset = new_data_loader.Tactile_Vision_dataset(params['Fruit_type'], params["Tactile_scale_ratio"],
    #                                                        params["Visual_scale_ratio"], params["video_length"],
    #                                                        data_path=params['Train_data_dir'])
    # test_dataset = new_data_loader.Tactile_Vision_dataset(params['Test_type'], params["Tactile_scale_ratio"],
    #                                                       params["Visual_scale_ratio"], params["video_length"],
    #                                                       data_path=params['Train_data_dir'])
    ...
    train_dataset = thresh_data_loader.Tactile_Vision_dataset(params['Fruit_type'], params["Tactile_scale_ratio"],
                                                           params["Visual_scale_ratio"], params["video_length"],
                                                           data_path=params['Train_data_dir'])
    test_dataset = thresh_data_loader.Tactile_Vision_dataset(params['Test_type'], params["Tactile_scale_ratio"],
                                                          params["Visual_scale_ratio"], params["video_length"],
                                                          data_path=params['Train_data_dir'])
    train_data_loader = DataLoader(train_dataset, batch_size=params['batch_size'], shuffle=True,
                                   num_workers=params['num_workers'])
    ...
    if 'save_dir' in params.keys():
        ...
        state_dict = {'model': NN_model.state_dict(), 'optimizer': optimizer.state_dict()}
        torch.save(state_dict, model_path)
        plot_loss_accuracy(train_loss, test_loss, train_acc, test_acc, exp_save_dir, colors=['blue'],loss_legend_loc='upper center', acc_legend_loc='upper left')
