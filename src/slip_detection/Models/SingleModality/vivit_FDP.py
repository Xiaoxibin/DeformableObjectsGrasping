...
class UniformPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, T, W  # W: 经过卷积投影后生成的图像的宽度上的 patch 数量
...
class VIVIT(nn.Module):
    def __init__(self,img_size, patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio,qkv_bias,qk_scale, drop_rate, attn_drop_rate, drop_path_rate,num_frames,dropout, use_gpu=False):
        ...
        # (           (120,160),  (12,16),     3,          2,          256,     8,      16,         14,     False,   None,        0,           0,              0,          14,       0,         False)
        super().__init__()
...
if __name__ == '__main__':
    ...
    x = torch.rand(16, 3, 8, 120, 160)  # (batch_size, in_chans, depth, height, width)
    ...
    vivit = VIVIT(img_size=(120,160), patch_size=(12,16), in_chans=3, num_classes=2, embed_dim=128, depth=1, num_heads=4, mlp_ratio=4, qkv_bias=False, qk_scale=None, drop_rate=0, attn_drop_rate=0., drop_path_rate=0.,  num_frames=8, dropout=0., use_gpu=use_gpu)
    if(use_gpu):
        vivit = vivit.to('cuda')
    out = vivit(x)  # out 是一个大小为 (batch_size, num_classes) 的张量，表示模型对输入数据的预测结果
    print(out)
