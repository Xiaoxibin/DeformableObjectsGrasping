...
sys.path.append("..")
from Models.SingleModality import basic_CNN, swin_transformer, vivit, timeSformer_orig, vivit_FDP
# Todo: add other model architecture and put them under ../Models

# Build and return the model here based on the configuration.
def get_model(config_data,use_gpu):
    model_type = config_data['Model_name']       # vivit_fdp
    Image_width = config_data['Image_width']     # 640
    Image_height = config_data['Image_height']   # 480
    video_length = config_data['video_length']   # 14
    Scale_ratio = config_data['scale_ratio']     # 0.25
    resized_width = Image_width * Scale_ratio    # 160
    resized_height = Image_height * Scale_ratio  # 120
    Model = None
    # You may add more parameters if you want
    # Based on the Image_width, Image_height, Scale_ratio, video_length, some hyperparameter can be computed here
    if model_type == 'basic_CNN':  # the benchmark for comparison
        ...
        Model = vivit_FDP.VIVIT(img_size=img_size, patch_size=patch_size, in_chans=in_chans,num_classes=num_cls, embed_dim=emb_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None, drop_rate=dropout, attn_drop_rate=attn_dropout, drop_path_rate=dropout, num_frames=num_frames, dropout=0.,use_gpu=use_gpu)
    ...
    elif model_type == 'timeSformer_orig': 
        img_size  = (int(resized_height),int(resized_width))
        patch_size = (12, 16)
        in_chans = 3
        num_cls = 2
        ...
        # if some parameters need to be computed, add them here:
        Model = timeSformer_orig.TimeSFormer(img_size=img_size, patch_size=patch_size, in_chans=in_chans,num_classes=num_cls, embed_dim=emb_dim, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None, drop_rate=dropout, attn_drop_rate=attn_dropout, drop_path_rate=dropout,hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=num_frames, attention_type='divided_space_time', dropout=0.,use_gpu=use_gpu)
